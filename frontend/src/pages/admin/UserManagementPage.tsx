...
export const UserManagementPage = () => {
    ...
    const handleAction = (type: 'ban' | 'unban' | 'reset' | 'role', user: typeof mockUsers[0]) => {
        let config: any = {
            isOpen: true,
            onConfirm: () => {
                console.log(`${type} action confirmed for ${user.username}`);
                closeDialog();
            }
        };
        ...
        switch (type) {
            ...
            case 'reset':
                config = {
                    ...
                    message: `Are you sure you want to reset the password for ${user.username}? A temporary password will be sent to their email.`,
            ...
            case 'role':
                config = {
                    ...
                    message: `You are about to modify the role for ${user.username}. This might change their access level on the platform.`,
                    confirmText: "Manage Roles",
                    variant: 'info',
                };
                break;
        }
                                    ...
                                    <tr key={user.id} className="hover:bg-gray-50/50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <Link to={ROUTES.ADMIN_USER_DETAIL(user.id)} className="relative shrink-0">
                                                    <img src={user.avatarUrl} alt={user.username} className="w-10 h-10 rounded-full border border-gray-100 object-cover" />
                                                    <div className={cn(
                                        ...
                                        <td className="px-6 py-4">
                                            {/* <div className="flex items-center justify-end gap-1">
                                                ...
                                                <ActionButton onClick={() => handleAction('reset', user)} icon={Key} title="Reset Pass" color="orange" />
                                                <ActionButton onClick={() => handleAction(user.status === "ACTIVE" ? 'ban' : 'unban', user)} icon={user.status === "ACTIVE" ? UserX : CheckCircle2} title={user.status === "ACTIVE" ? "Ban User" : "Unban"} color={user.status === "ACTIVE" ? "red" : "green"} />
...
const RoleBadge = ({ role }: { role: string }) => {
    const variants: Record<string, { bg: string; text: string; icon: any }> = {
        ...
        MODERATOR: { bg: "bg-purple-50", text: "text-purple-600", icon: Shield },
        TRANSLATOR: { bg: "bg-blue-50", text: "text-blue-600", icon: Shield },
...
const ActionButton = ({ icon: Icon, title, color }: { icon: any; title: string; color: string }) => {
