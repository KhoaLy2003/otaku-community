...
-   **Description:** Retrieves a paginated list of posts by a specific user, optionally filtered by status.
-   **Method:** `GET`
-   **URL:** `/api/posts/user/{userId}`
-   **URL Parameters:**
    -   `userId` (UUID): The ID of the user whose posts to retrieve.
-   **Query Parameters:**
    -   `status` (PostStatus): Optional filter for post status (`DRAFT`, `PUBLISHED`).
    -   `page` (int): Page number (1-based). Defaults to 1.
    -   `size` (int): Page size (max 20). Defaults to 20.
-   **Responses:**
